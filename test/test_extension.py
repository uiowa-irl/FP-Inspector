...
# Document DB calls and property sets
DOCUMENT_TEST_URL = u"%s/document_property_test_page.html" % utilities.BASE_TEST_URL

DOCUMENT_CALLS = {
    (DOCUMENT_TEST_URL, 'Document.createComment', 'call',
     '', '{"0":"This is a not-so-secret comment in your document"}'),
    (DOCUMENT_TEST_URL, 'Node.appendChild', 'call',
     '', '{"0":"{}"}'),
    (DOCUMENT_TEST_URL, 'Document.createDocumentFragment', 'call',
     '', None),
    (DOCUMENT_TEST_URL, 'Document.createElement', 'call',
     '', '{"0":"div"}'),
    (DOCUMENT_TEST_URL, 'Document.createTextNode', 'call',
     '', '{"0":"text"}')
}
...
class TestExtension(OpenWPMTest):
    ...
    def test_document_calls(self):
        db = self.visit('/document_property_test_page.html')
        # Check that all calls and methods are recorded
        rows = db_utils.get_javascript_entries(db)
        observed_rows = set()
        for row in rows:
            item = (row['script_url'], row['symbol'], row['operation'],
                    row['value'], row['arguments'])
            observed_rows.add(item)
        assert DOCUMENT_CALLS == observed_rows
